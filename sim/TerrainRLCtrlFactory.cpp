...
#include "sim/BipedSymStepStochController3D.h"
#include "sim/CtTargetTerrController.h"
#include "sim/CtTargetSoccerController.h"
#include "sim/TerrainRLCrowdCharController.h"
...
bool cTerrainRLCtrlFactory::BuildController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	...
	case eCharCtrlCtPDPhaseTarget:
		...
		break;
	case eCharCtrlCtPDGRF:
		succ = BuildeCtPDGRFController(params, out_ctrl);
		break;
	...
	case eCharCtrlMultiCharWaypoint:
	 	...
	 	break;
	case eCharCtrlMultiCharWaypointVel:
		succ = BuildMultiCharWaypointVelController(params, out_ctrl);
		break;
	case eCharCtrlCrowd:
		succ = BuildCrowdCharController(params, out_ctrl);
		break;
	case eCharVelCtrlCrowd:
		succ = BuildCrowdCharVelController(params, out_ctrl);
		break;
	case eCharVelCtrlCrowdVelGoal:
		succ = BuildCrowdCharVelGoalVelController(params, out_ctrl);
		break;
...
bool cTerrainRLCtrlFactory::BuildeCtPDGRFController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	const double update_period = 1 / params.mCtQueryRate;
	bool succ = true;

	std::shared_ptr<cCtPDGRFController> ctrl = std::shared_ptr<cCtPDGRFController>(new cCtPDGRFController());
	ctrl->SetGround(params.mGround);
	ctrl->Init(params.mChar.get(), params.mGravity, params.mCtrlParamFile);
	ctrl->SetUpdatePeriod(update_period);
...
bool cTerrainRLCtrlFactory::BuildMultiCharWaypointController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	bool succ = true;
	...
	auto step_ctrl = std::dynamic_pointer_cast<cBipedStepController3D>(llc);
	auto ctrl = std::shared_ptr<cMultiCharWaypointController>(new cMultiCharWaypointController());
	// auto ctrl = std::shared_ptr<cTerrainRLCrowdCharController>(new cTerrainRLCrowdCharController());
...
bool cTerrainRLCtrlFactory::BuildMultiCharWaypointVelController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	bool succ = true;
	...
	auto step_ctrl = std::dynamic_pointer_cast<cBipedStepController3D>(llc);
	auto ctrl = std::shared_ptr<cMultiCharWaypointVelController>(new cMultiCharWaypointVelController());
...
bool cTerrainRLCtrlFactory::BuildCrowdCharController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	bool succ = true;
...
bool cTerrainRLCtrlFactory::BuildCrowdCharVelController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	bool succ = true;
	...
	auto step_ctrl = std::dynamic_pointer_cast<cBipedStepController3D>(llc);
	auto ctrl = std::shared_ptr<cTerrainRLCrowdCharVelController>(new cTerrainRLCrowdCharVelController());
...
bool cTerrainRLCtrlFactory::BuildCrowdCharVelGoalVelController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	bool succ = true;
	...
	auto step_ctrl = std::dynamic_pointer_cast<cBipedStepController3D>(llc);
	auto ctrl = std::shared_ptr<cTerrainRLCrowdCharVelGoalVelController>(new cTerrainRLCrowdCharVelGoalVelController());
...
bool cTerrainRLCtrlFactory::BuildCrowdCharVelGoalVelPosController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	bool succ = true;
...
bool cTerrainRLCtrlFactory::BuildBipedSymStepStochController3D(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl)
{
	const double update_period = 1 / params.mCtQueryRate;
	bool succ = true;

	auto ctrl = std::shared_ptr<cBipedSymStepStochController3D>(new cBipedSymStepStochController3D());
	ctrl->SetGround(params.mGround);
	ctrl->Init(params.mChar.get(), params.mGravity, params.mCtrlParamFile);
	ctrl->SetUpdatePeriod(update_period);
	ctrl->SetCycleDur(params.mCycleDur);
