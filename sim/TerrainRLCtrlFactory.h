...
public:
	...
	enum eCharCtrl
	{
		...
		eCharVelCtrlCrowdVelPosGoal,
		eCharCtrlWaypointVel,
		eCharCtrlSoccer,
		eCharCtrlBiped3DStep,
		eCharCtrlBiped3DSymStep,
		eCharCtrlBiped3DSymStepStoch,
...
protected:
	...
	/// Controllers that include ground reaction forces on end effectors.
	...
	static bool BuildCrowdCharVelGoalVelPosController(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl);
	...
	static bool BuildBipedSymStepController3D(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl);
	static bool BuildBipedSymStepStochController3D(const tCtrlParams& params, std::shared_ptr<cCharController>& out_ctrl);
