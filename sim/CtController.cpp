	...
	for (int i = 0; i < num_parts; ++i)
	{
		...
		if (mChar->IsValidBodyPart(part_id))
		{
			...
			if (Is3D())
			{
				...
				if ( mExpParams.stateRotationType == AxisAngleRot)
				{
					// std::cout << "Using PD_Humanoid_2D_Symetric_NoPhase_AxisAngle_v0 " << std::endl;
					tVector axis;
					double theta;
					cMathUtil::QuaternionToAxisAngle(curr_quat, axis, theta);
					out_pose.segment(idx, rot_dim) = cMathUtil::AxisAngleToVec(axis, theta).segment(0, rot_dim);
				}
				else
				{
