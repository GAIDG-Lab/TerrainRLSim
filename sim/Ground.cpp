...
cGround::eClass cGround::GetClassFromType(eType ground_type)
{
	...
	switch (ground_type)
	{
		...
		case eTypeDynamicCharacters3D:
			...
			std::cout << "Setting dynamic char ground class" << std::endl;
			break;
...
void cGround::SampleHeightVel(cSimCharacter* _char, const tVector& pos, double& out_h, tVector& out_vel,
										bool& out_valid_sample) const
{
	SampleHeightVel(pos, out_h, out_vel, out_valid_sample);
}
...
int cGround::GetNumObstacles() const
{
	return static_cast<int>(0);
}

const cSimObj& cGround::GetObj(int i) const
{
	/// This is a poor hack to get things to compile...
