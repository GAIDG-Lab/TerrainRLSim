...
void cWorld::ParseRenderMode(const std::string& render_mode_str, eRenderMode& out_sim_mode)
{
	if (render_mode_str == "2d")
	{
		out_sim_mode = eRenderMode2D;
	}
	else if (render_mode_str == "3d")
	{
		out_sim_mode = eRenderMode3D;
	}
	else if (render_mode_str == "3d_uber")
	{
		out_sim_mode = eRenderModeUber3D;
	}
	else
	{
		std::cout << "Render mode not supported: "  << render_mode_str << std::endl;
	}
}
...
cWorld::tConstraintHandle cWorld::AddJoint(cSimObj* obj0, cSimObj* obj1, const tJointParams& params)
{
	...
	if (!is_root)
	{
		...
		// 	handle = AddFixedConstraint(obj0, obj1, params);
		// 	break;
		default:
			assert(false);
...
cWorld::eRenderMode cWorld::GetRenderMode() const
{
	return mParams.mRenderMode;
}
...
std::unique_ptr<btCylinderShape> cWorld::BuildCylinderShape(double radius, double height) const
{
	btScalar scale = static_cast<btScalar>(GetScale());
	std::unique_ptr<btCylinderShape> shape = std::unique_ptr<btCylinderShape>(new btCylinderShape(btVector3(static_cast<btScalar>(scale * radius),
...
{
	assert(obj.GetShape() == cSimObj::eShapeCapsule);
	const btCapsuleShape* shape = reinterpret_cast<const btCapsuleShape*>(obj.GetCollisionShape().get());
	double scale = GetScale();
	double r = shape->getRadius();
...
{
	assert(obj.GetShape() == cSimObj::eShapePlane);
	const btStaticPlaneShape* shape = reinterpret_cast<const btStaticPlaneShape*>(obj.GetCollisionShape().get());
	double scale = GetScale();
...
{
	assert(obj.GetShape() == cSimObj::eShapeSphere);
	const btSphereShape* ball_shape = reinterpret_cast<const btSphereShape*>(obj.GetCollisionShape().get());
...
{
	assert(obj.GetShape() == cSimObj::eShapeCylinder);
	const btCylinderShape* shape = reinterpret_cast<const btCylinderShape*>(obj.GetCollisionShape().get());
	double scale = GetScale();
	double r = shape->getRadius();
...
/*
cWorld::tConstraintHandle cWorld::AddFixedConstraint(cSimObj* obj0, cSimObj* obj1, const tJointParams& params)
{
	btScalar scale = static_cast<btScalar>(GetScale());
	auto sim_link = obj0; // dynamic_cast<const cSimBodyLink*>(mParent.get());
	// if (sim_link == nullptr)
	{
