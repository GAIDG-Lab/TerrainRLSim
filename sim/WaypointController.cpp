...
void cWaypointController::SetLLC(const std::shared_ptr<cBipedStepController3D>& ctrl)
{
	...
	auto sym_step_ctrl = std::dynamic_pointer_cast<cBipedSymStepController3D>(ctrl);
	bool sym_llc = sym_step_ctrl != nullptr;
	if (sym_llc)
	{
		mFlipJointOrder = &(sym_step_ctrl->GetFlipJointOrder());
	}
}
