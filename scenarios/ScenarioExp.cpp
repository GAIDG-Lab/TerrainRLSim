...
void cScenarioExp::ParsePoliRotStateMode(const std::string& poli_state_mode_str, cCharController::eStateRotType& out_sim_mode)
{
	if (poli_state_mode_str == "quaternion")
	{
		out_sim_mode = cCharController::eStateRotType::QuaternionRot;
	}
	else if (poli_state_mode_str == "axisangle")
	{
		out_sim_mode = cCharController::eStateRotType::AxisAngleRot;
	}
	else if (poli_state_mode_str == "euler")
	{
		out_sim_mode = cCharController::eStateRotType::EulerRot;
	}
	else
	{
		std::cout << "Poli State mode not supported: "  << poli_state_mode_str << std::endl;
	}
}
...
void cScenarioExp::ParseArgs(const std::shared_ptr<cArgParser>& parser)
{
	...
	std::string rot_state_mode_str = "";
	parser->ParseString("poli_state_rot_type", rot_state_mode_str);
	cScenarioExp::ParsePoliRotStateMode(rot_state_mode_str, mExpParams.stateRotationType);
