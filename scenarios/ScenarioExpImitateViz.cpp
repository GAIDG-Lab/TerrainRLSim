#include "ScenarioExpImitateViz.h"
...
double cScenarioExpImitateViz::CalcReward() const
{

	double pose_w = _pose_w; // pose error weight
	double vel_w = _vel_w; // pose velocity error
	double end_eff_w = _end_eff_w; // weight for end effector error
	double root_w = _root_w; //
	double com_w = _com_w; /// com velocity error weight
	...
	if (!fallen)
	{
		...
		/// Computes the COM velocity of the kinematic character
		...
		// printf("Error: %.5f, %.5f, %.5f, %.5f\n", root_pos_err, 0.1 * root_rot_err, 0.01 * root_vel_err, 0.001 * root_ang_vel_err);
		// printf("Root Reward: %.5f\n", root_reward);
		// printf("reward: %.5f, %.5f, %.5f, %.5f, %.5f\n", pose_reward, vel_reward, end_eff_reward, root_reward, com_reward);
...
cScenarioExpImitateViz::cScenarioExpImitateViz()
{
	mMotionFile = "";
...
void cScenarioExpImitateViz::ParseArgs(const std::shared_ptr<cArgParser>& parser)
{
	...
	succ = parser->ParseDouble("root_pose_weight", _root_w);
	succ = parser->ParseDouble("com_velocity_error_weight", _com_w);

	succ = parser->ParseDouble("time_warp_min", _time_warp_min);
	succ = parser->ParseDouble("time_warp_max", _time_warp_max);
}

void cScenarioExpImitateViz::Init()
{
	...
	{
		kin_char_tmp->BuildBody(mWorld);
		kin_char_tmp->SetGround(mGround);
	}
	InitJointWeights();
...
const std::shared_ptr<cKinCharacter>& cScenarioExpImitateViz::GetKinChar() const
{
	return mKinChar;
}

void cScenarioExpImitateViz::EnableRandStateReset(bool enable)
{
	mEnableRandStateReset = enable;
}

bool cScenarioExpImitateViz::EnabledRandStateReset() const
{
	return mEnableRandStateReset;
}

bool cScenarioExpImitateViz::HasFallen() const
{
	bool fallen = cScenarioSimChar::HasFallen();
	return fallen;
}

std::string cScenarioExpImitateViz::GetName() const
{
	return "Imitate Exploration";
}

void cScenarioExpImitateViz::CalcJointWeights(const std::shared_ptr<cSimCharacter>& character, Eigen::VectorXd& out_weights) const
{
...
void cScenarioExpImitateViz::SetupControllerParams(cTerrainRLCtrlFactory::tCtrlParams& out_params) const
{
...
void cScenarioExpImitateViz::UpdateCharacter(double time_step)
{
...
void cScenarioExpImitateViz::UpdateKinChar(double time_step)
{
	mKinChar->Update(time_step);
	//SyncCharacters(); // hack hack hack
}

void cScenarioExpImitateViz::UpdateTrackController()
{
...
void cScenarioExpImitateViz::ResetParams()
{
	cScenarioExpCacla::ResetParams();
}

void cScenarioExpImitateViz::ResetCharacters()
{
...
void cScenarioExpImitateViz::ResetKinChar()
{
	mKinChar->Reset();
	if (EnabledRandStateReset())
	{
		// TODO: Make this a scenario parameters in the future.
		double prob = mRand.RandDouble(0, 1.0);
		// std::cout << " reset rand time prob: " << prob << std::endl;
		// if (prob > 0.5)
		{
...
//			std::cout << "cKinTree::eDrawShapeColorR before: " << mChar->GetDrawShapeDefs().row(i)[cKinTree::eDrawShapeColorR] << std::endl;
...
//			std::cout << "cKinTree::eDrawShapeColorR after: " << mChar->GetDrawShapeDefs().row(i)[cKinTree::eDrawShapeColorR] << std::endl;
...
void cScenarioExpImitateViz::SyncCharacters()
{
...
bool cScenarioExpImitateViz::EnableSyncChar() const
{
	return true;
}

void cScenarioExpImitateViz::InitJointWeights()
{
	CalcJointWeights(mChar, mJointWeights);
}

int cScenarioExpImitateViz::GetNumWarmupCycles() const
{
	return gNumWarmupCycles;
}

bool cScenarioExpImitateViz::EndEpisode() const
{
...
double cScenarioExpImitateViz::CalcRandKinResetTime()
{
...
double cScenarioExpImitateViz::CalcRandTimeWarping()
{
