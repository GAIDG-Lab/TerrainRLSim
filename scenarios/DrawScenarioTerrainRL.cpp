...
void cDrawScenarioTerrainRL::InitRenderResources()
{
...

	{
...
#ifdef USE_OpenGLES
		succ &= mShaderMesh->BuildShader("render/shaders/GLES/Mesh_VS.gles", "render/shaders/GLES/Lighting_PS.gles");
#else
		succ &= mShaderMesh->BuildShader("render/shaders/Mesh_VS.glsl", "render/shaders/Lighting_PS.glsl");
#endif
		if (not succ)
		{
			std::cout << "Failed building shaderMesh" << std::endl;
		}
		...
		// mShaderMesh->BindShaderUniform(mTexResHandle, "gTexRes"); // TODO: Because it is not used in the shader...
		mShaderMesh->BindShaderUniform(mMaterialDataHandle, "gMaterialData");
		// mShaderMesh->BindShaderUniform(mProjectionMatHandle, "projectionMat");
		cDrawUtil::AddShader(mShaderMesh);
...
#ifdef USE_OpenGLES
		succ &= mShaderDepth->BuildShader("render/shaders/GLES/Mesh_VS.gles", "render/shaders/GLES/Depth_PS.gles");
#else
		succ &= mShaderDepth->BuildShader("render/shaders/Mesh_VS.glsl", "render/shaders/Depth_PS.glsl");
#endif
		// cDrawUtil::AddShader(mShaderDepth);
	}
...
#ifdef USE_OpenGLES
		succ &= mShaderGBuffer->BuildShader("render/shaders/GLES/Mesh_VS.gles", "render/shaders/GLES/GBuffer_PS.gles");
#else
		succ &= mShaderGBuffer->BuildShader("render/shaders/Mesh_VS.glsl", "render/shaders/GBuffer_PS.glsl");
#endif
		cDrawUtil::AddShader(mShaderGBuffer);
	}

	{
...
#ifdef USE_OpenGLES
		succ &= mLambertShaderMesh->BuildShader("render/shaders/GLES/Mesh_VS.gles", "render/shaders/GLES/Lighting_Lambert_PS.gles");
#else
		...
		// mLambertShaderMesh->BindShaderUniform(mLambertProjectionMatHandle, "projectionMat");
		cDrawUtil::AddShader(mLambertShaderMesh);
...
void cDrawScenarioTerrainRL::SetupMeshShaderUber3D()
{
	...
	// mShaderMesh->SetUniform2(mTexResHandle, tex_res);
...
void cDrawScenarioTerrainRL::DrawObstacles() const
{
