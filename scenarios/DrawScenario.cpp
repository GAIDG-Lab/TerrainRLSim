...
void cDrawScenario::ParseArgs(const std::shared_ptr<cArgParser>& parser)
{
...
void cDrawScenario::EnableFilmstrip(bool enable)
{
	mEnableFilmStrip = enable;
}

void cDrawScenario::ParseCamTrackMode(const std::shared_ptr<cArgParser>& parser, eCamTrackMode& out_mode) const
{
	std::string str = "";
...
void cDrawScenario::UpdateCameraTracking()
{
	...
	else if (mCamTrackMode == eCamTrackModeXZ
		|| mCamTrackMode == eCamTrackModeY)
	{
		tVector track_pos = GetCamTrackPos();
