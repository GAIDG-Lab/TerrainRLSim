#include "ScenarioExpImitateMocap.h"
...
double cScenarioExpImitateMocap::CalcReward() const
{
	double pose_w = 0.5;
	double vel_w = 0.05;
	...
	if (!fallen)
	{
		...
		for (int j = root_id + 1; j < num_joints; ++j)
		{
			...
			if (mAddTorquePenalty)
			{
				...
				// std::cout << "torque_err: " << j << " sum: " << torque_err << std::endl;
		...
		{
			reward += exp(-err_scale * pose_scale * torque_err) * end_eff_w;
		}
	}
...
void cScenarioExpImitateMocap::ParseArgs(const std::shared_ptr<cArgParser>& parser)
{
	...
	parser->ParseBool("add_torque_penalty", mAddTorquePenalty);

}


void cScenarioExpImitateMocap::Init()
{
	bool succ = BuildKinCharacter(mKinChar);
	bool succ2 = BuildKinCharacter(mKinChar2);

	int numFrames = mKinChar->GetMotion().GetNumFrames();
	printf("ScenarioExpImitateMocap.cpp Init KinCharacter numFrames: %d\n", numFrames);
...
const std::shared_ptr<cKinCharacter>& cScenarioExpImitateMocap::GetKinChar() const
{
	//printf("ScenarioExpImitateMocap::GetKinChar\n");
	return mKinChar;
}

const std::shared_ptr<cKinCharacter>& cScenarioExpImitateMocap::GetKinChar2() const
{
	return mKinChar2;
}

const std::vector<std::shared_ptr<cKinCharacter>>& cScenarioExpImitateMocap::GetKinChars() const
{
	return mKinChars;
}

void cScenarioExpImitateMocap::EnableRandStateReset(bool enable)
{
	mEnableRandStateReset = enable;
}

bool cScenarioExpImitateMocap::EnabledRandStateReset() const
{
	return mEnableRandStateReset;
}

bool cScenarioExpImitateMocap::HasFallen() const
{
	bool fallen = cScenarioSimChar::HasFallen();
	return fallen;
}

std::string cScenarioExpImitateMocap::GetName() const
{
	return "Imitate Exploration";
}

void cScenarioExpImitateMocap::CalcJointWeights(const std::shared_ptr<cSimCharacter>& character, Eigen::VectorXd& out_weights) const
{
...
void cScenarioExpImitateMocap::SetupControllerParams(cTerrainRLCtrlFactory::tCtrlParams& out_params) const
{
	cScenarioExpCacla::SetupControllerParams(out_params);
	out_params.mCycleDur = mKinChar->GetMotionDuration();
}

bool cScenarioExpImitateMocap::BuildKinCharacter(std::shared_ptr<cKinCharacter>& out_char) const
{
...
void cScenarioExpImitateMocap::UpdateCharacter(double time_step)
{
...
void cScenarioExpImitateMocap::UpdateKinChar(double time_step)
{
	//mKinChar->Update(time_step); //time_step=0.001667
	//SyncCharacters(); // hack hack hack
}

void cScenarioExpImitateMocap::UpdateTrackController()
{
	//printf("ScenarioExpImitateMocap::UpdateTrackController\n");
...
void cScenarioExpImitateMocap::ResetParams()
{
	cScenarioExpCacla::ResetParams();
}

void cScenarioExpImitateMocap::ResetCharacters()
{
...
void cScenarioExpImitateMocap::ResetKinChar()
{
	mKinChar->Reset();
...
void cScenarioExpImitateMocap::SyncCharacters()
{
...
bool cScenarioExpImitateMocap::EnableSyncChar() const
{
	return true;
}

void cScenarioExpImitateMocap::InitJointWeights()
{
	CalcJointWeights(mChar, mJointWeights);
}

int cScenarioExpImitateMocap::GetNumWarmupCycles() const
{
	return gNumWarmupCycles;
}

bool cScenarioExpImitateMocap::EndEpisode() const
{
...
double cScenarioExpImitateMocap::CalcRandKinResetTime()
{
